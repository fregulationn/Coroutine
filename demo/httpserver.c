...
#define SERVER_STRING "Server: hxh_httpd/0.1.0\r\n"
#define STDIN   0
#define STDOUT  1
#define STDERR  2
...
#define socket coroutine_socket
#define accept coroutine_accept
#define read(a, b, c) coroutine_read(a, b, c)
#define write(a, b, c) coroutine_write(a, b, c)
...
int startup(uint16_t *port)
{
    ...
    if (*port == 0)  /* if dynamically allocating a port */
    {
        ...
        if (getsockname(httpd, (struct sockaddr *)&addr, &addrlen) == -1)
...
void server(void *arg)
{
    ...
    while (1)
    {
        ...
        *client_sock = accept(server_sock,
		...
		coroutine_t *read_co;
		coroutine_create(&read_co, accept_request, client_sock);
...
int main(int argc, char *argv[]) {
	...
	coroutine_t *co = NULL;
	coroutine_create(&co, server, 0); ////////no run
	

	schedule_running(); //run
